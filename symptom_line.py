...
df_symptom = df_symptom.loc[df_symptom['Symptom Type'].isin(top5),:].groupby(['Issue Created At','Symptom Type'],sort=False).count().expanding().mean().reset_index().set_index('Issue Created At')
print(df_symptom)
fig = px.line(df_symptom, x=df_symptom.index.values, y='Line', color='Symptom Type')
fig.show()